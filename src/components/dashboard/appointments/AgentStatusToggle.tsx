...
export default function AppointmentStatusToggleRequest({
  ...
  const handleToggle = async () => {
    setLoading(true);
    try {
      await updateAppointmentStatusFinal(appointmentId, {
  ...
  const statusOptions = useMemo(() => {
    const allOptions = Object.entries(agentBookingStatusMap).map(
      ([value, label]) => ({
        value,
        label,
      })
  ...
  console.log(agentBookingStatusMap);
    ...
    <div className="rounded-lg bg-white max-w-md mx-auto">
      ...
      <p className="text-sm text-gray-600 text-center mb-4">
        ...
        <span className={`${style} !bg-white`}>
          {" "}
          الحالة الحالية هي {agentBookingStatusMap[currentStatus]}
        </span>
      </p>
      ...
      <div className="flex justify-end gap-3 pt-6">
        ...
        >
          {loading
            ? "جارٍ التنفيذ..."
            : `تأكيد التغيير إلى "${agentBookingStatusMap[selectedStatus]}"`}
        </button>
      </div>
    </div>
  );
