...
export default function InfoCell({
  ...
  const imageClass = `w-10 h-10 object-cover ${
    imageRounded === "full" ? "rounded-full" : "rounded-lg"
  }`;
        ...
        {subtitle &&
          (subtitleContent.tooltip ? (
            <Tooltip text={subtitleContent.tooltip}>
              <span className={fullSubtitleClass}>
                {subtitleContent.display}
              </span>
            </Tooltip>
          ) : (
