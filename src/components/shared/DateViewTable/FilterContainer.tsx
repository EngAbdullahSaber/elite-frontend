...
export default function FilterContainer({
  ...
  const [allFilters, setAllFilters] = useState(() => {
    ...
    initial.sort =
      searchParams.get("sort") ?? defaultSort ?? sortFields[0]?.value;
      ...
      if (filter.type === "dateRange") {
        ...
        initial[`${filter.key}_from`] =
          ...
          (defaultRange.startDate
            ? format(defaultRange.startDate, "yyyy-MM-dd")
            : "");

        initial[`${filter.key}_to`] =
          toParam ??
          (defaultRange.endDate
            ? format(defaultRange.endDate, "yyyy-MM-dd")
            : "");
      }
    });
    ...
    filters.forEach((filter) => {
      ...
      if (filter.type === "dateRange") {
        ...
        if (defaultRange.startDate) {
          reset[`${filter.key}_from`] = format(
            defaultRange.startDate,
            "yyyy-MM-dd"
          );
        }
        if (defaultRange.endDate) {
          reset[`${filter.key}_to`] = format(
            defaultRange.endDate,
            "yyyy-MM-dd"
          );
        }
